...
*/
	...
	list($total_players, $total_kills, $total_headshots) = $db->fetch_row($result);
	...
	";
	$result = $db->query($query);
	list($total_servers) = $db->fetch_row($result);
...
<?php
		...
		for ($i = 0; $i < count($servers); $i++)
		{
			...
			$kills = $total_kills;#$rowdata['kills'];
			$headshots = $total_headshots;#$rowdata['headshots'];
