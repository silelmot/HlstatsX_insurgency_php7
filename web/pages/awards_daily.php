...
if ( !defined('IN_HLSTATS') )
{
	...
	$resultAwards = $db->query("
		...
		ORDER BY
			hlstats_Awards.negative ASC , hlstats_Awards.name 
	");
...
<?php
	...
	while ($r = $db->fetch_array($resultAwards))
	{
		...
		if ($r['d_winner_id'] > 0) {
			...
			$weapon = '<a href="hlstats.php?mode=dailyawardinfo&amp;award='.$r['awardId']."&amp;game=$game\"><img style=\"opacity:0.3\" src=\"$img\" alt=\"".$r['code'].'" /></a>';
			$achvd = "<em>No Award Winner</em>";
			$wincount= "0";
			echo "<td style=\"text-align:center;vertical-align:top;width:$colwidth%;color:grey;\">
			<strong>".$r['name'].'</strong><br /><br />'
			."$weapon<br /><br />"
			."$achvd<br />"
			.'<span class="fSmall">'.$wincount. ' ' . htmlspecialchars($r['verb']).'</span>
