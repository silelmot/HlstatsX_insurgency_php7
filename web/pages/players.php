			...
			(
				...
				(
					...
					(
						'lastName',
						'Player',
						'width=20&flag=1&link=' . urlencode('mode=playerinfo&amp;player=%k')
					),
					new TableColumn
					(
						'skill',
					...
					(
						'rang',
						'Rang',
						'width=5&align=center&type=rankimg'
					),
					new TableColumn
					(
						'awards',
		...
		if ($rank_type == "0")
		{
			...
			("
				SELECT
					...
					COUNT(hlstats_Awards.g_winner_id) AS awards,
                                        (       SELECT hlstats_Ranks.image 
                                                FROM hlstats_Ranks 
                                                WHERE hlstats_Ranks.minKills <= hlstats_Players.kills 
				...
				FROM
					hlstats_Players
				INNER JOIN hlstats_Awards ON 
					hlstats_Players.playerId = hlstats_Awards.g_winner_id
					OR hlstats_Awards.g_winner_id IS NULL
				WHERE
